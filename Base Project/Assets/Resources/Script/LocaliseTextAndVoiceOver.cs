...
public class LocaliseTextAndVoiceOver : MonoBehaviour
{
    ...
    void GetFontForText()
    {
        if (PlayerPrefs.GetString("currentLanguage") != LanguageHandler.defaultLanguage)
        {
            GetComponentInChildren<Text>().font = AssetBundleManager.Instance.GetAssetBundle(PlayerPrefs.GetString("currentLanguage")).LoadAsset<Font>(fontName);
        }
        else
        {
